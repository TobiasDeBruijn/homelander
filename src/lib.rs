            ...
            .map(|input| match input {
                Input::Execute(execute) => {
                    let commands = execute.commands.into_iter()
                        .map(|command| command.devices.into_iter()
                                .map(|device| device.id)
                                .map(|device_id| command.execution.iter()
                        ...
                        .map(|output| {
                            match output.status {
                                ...
                                CommandStatus::Offline | CommandStatus::Pending => {
                                    fulfillment::response::execute::Command {
                                        ids: vec![output.id],
                                        status: output.status,
                                        states: None,
                    ...
                    fulfillment::response::ResponsePayload::Execute(fulfillment::response::execute::Payload {
                        commands
                    })
                },
                Input::Sync => {
                    fulfillment::response::ResponsePayload::Sync(self.sync())
                },
                // TODO QUERY
            })
            .collect::<Vec<_>>()
